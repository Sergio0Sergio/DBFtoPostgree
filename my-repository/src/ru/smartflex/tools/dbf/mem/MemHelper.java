...
public class MemHelper {
	...
	private static int STARTNO = 2305508; /*
										 * 1st Mar, 1600 (nominally, not
										 * actually)
										 */
	private static int STARTYY = 1600;
	private static int STARTMM = 3;
	private static int STARTDD = 1;
	...
	private static long readLong(byte[] record) {
		...
		l = (((long) record[0] << 56) + ((long) (record[1] & 255) << 48)
	 ...
	 * @return
	 */
	protected static Date getDate(double num) {
		Date date = null;
	...
	protected static String getString(byte[] record, int off, int len,
			String enc) throws UnsupportedEncodingException {
		int amount = getAmount(record, off, len);
		return new String(record, off, amount, enc);
	}

	protected static String getFieldName(byte[] record, int off, int len) {
		int amount = getAmount(record, off, len);
	 ...
	 * @return
	 */
	protected static boolean readFully(InputStream in, byte b[], int off,
			int len) {
		if (len <= 0)
